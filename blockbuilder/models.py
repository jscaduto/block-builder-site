...
class Phrase(models.Model):
    block_set_name = models.CharField(max_length=100,
                                      choices=BLOCK_SET_CHOICES,
                                      blank=False)
    regex_validator = RegexValidator(
        r'^[\sa-zA-Z4]*$',
        u"Phrase must ONLY contain letters and the number '4' to represent '4th'.",
        ValidationError)
    phrase_text = models.CharField(max_length=140,
                                   blank=False,
                                   validators=[validate_phrase_length,
                                               regex_validator])
